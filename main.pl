...
play(HousesWeight, BoardsWeight) :-
  initialize(GameState),
  play(GameState, HousesWeight, BoardsWeight).
...
play(GameState0, HousesWeight, BoardsWeight) :-
  select_and_make_move(GameState0, GameState1, HousesWeight, BoardsWeight),
  switch_player(GameState1, GameState2),
  !,
  play(GameState2, HousesWeight, BoardsWeight).

select_and_make_move(GameState0, GameState1, HousesWeight, BoardsWeight) :-
  display_board(GameState0),
  not(game_over(GameState0)),
  select_pit(GameState0, Pit, HousesWeight, BoardsWeight),
  make_move(Pit, GameState0, GameState1, HousesWeight, BoardsWeight).
...
select_pit(GameState, Pit, HousesWeight, BoardsWeight) :-
  GameState = game_state(_, _, bot),
  nl, writeln(['Opponent\'s turn']),
  find_best_move(GameState, Pit, HousesWeight, BoardsWeight),
...
make_move(Pit, GameState0, GameState2, HousesWeight, BoardsWeight) :-
  ...
  !,
  select_and_make_move(GameState1, GameState2, HousesWeight, BoardsWeight).
